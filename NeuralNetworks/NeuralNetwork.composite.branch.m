...
BackPropogateLayers[currentParameters_,neuronActivations_,finalLayerDelta_,OptionsPattern[]]:=Block[{delta,networkLayers,layerIndex},(
...
(* layerIndex refers to layer being back propogated across
   ie computing delta's for layerIndex-1 given layerIndex *)
      ...
      Timer["Backprop Layer "<>LayerName[currentParameters[[layerIndex]]],
         delta[[layerIndex-1]]=
            If[ToString[Head[currentParameters[[layerIndex]]]]!="DeCompose",
               BackPropogateLayer[
                  currentParameters[[layerIndex]],
                  delta[[layerIndex]],
                  neuronActivations[[layerIndex-1]],
                  neuronActivations[[layerIndex]]],
               sv=BackPropogateLayer[
                  currentParameters[[layerIndex]],
                  delta[[layerIndex]],
                  neuronActivations[[layerIndex-1]],
                  neuronActivations[[layerIndex]],currentParameters[[layerIndex-1]]]
         ](*Endif*)
;
      ]

      AbortAssert[Dimensions[delta[[layerIndex-1]]]==Dimensions[neuronActivations[[layerIndex-1]]]];
(* CHECK LATER  CAUSES SOME PROBLEM. WHAT?     delta[[layerIndex-1]]+=Sign[neuronActivations[[layerIndex-1]]]*xL1A; *)
   ];

   delta
)];

GradLayers[currentParameters_,inputs_,deltas_]:=
   MapThread[GradLayer,{currentParameters,inputs,deltas}]
...
NNGrad[currentParameters_,inputs_,targets_,lossF_,opts:OptionsPattern[]]:=(
   ...
   Prepend[
      ...
      If[ToString[Head[currentParameters[[1]]]]!="Composite",
         GradLayer[currentParameters[[1]],inputs,xDelta[[1]]],
         GradLayer[currentParameters[[1]],{Prepend[L[[1,1,1;;-2]],inputs[[All,1]]],Prepend[L[[1,2,1;;-2]],inputs[[All,2]]]},xDelta[[1]]]]
   ]](* NB Room for code improvement by redefining L to include input layer? *)
)
