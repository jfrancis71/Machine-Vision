...
(*UNTESTED*)
SyntaxInformation[Composite]={"ArgumentsPattern"->{_}};
Composite::usage = "Composite takes a list of 2 elements where each element is a neural network, ie a list.\n
The two lists need not be of equal length to reflect subnetworks of differing depths.\n
It is assumed that the final output stage outputs a pair of vectors.";
ForwardPropogateLayer[inputs_,Composite[{net1_,net2_}]]:=
   {ForwardPropogateLayers[inputs[[All,1]],net1],ForwardPropogateLayers[inputs[[All,2]],net2]};
BackPropogateLayer[Composite[{net1_,net2_}],postLayerDeltaA_,_,outputs_]:=
   {AbortAssert[3==4,"BackPropogateLayer::Composite UNIMPLEMENTED"];BackPropogateLayers[net1,outputs[[1,1]],postLayerDeltaA[[1,1]]],
    BackPropogateLayers[net2,outputs[[2,1]],postLayerDeltaA[[2,1]]]};
GradLayer[Composite[{net1_,net2_}],layerInputs_,layerOutputDelta_]:={
   GradLayers[net1,zf1=layerInputs[[1]],layerOutputDelta[[1]]],
   GradLayers[net2,zf2=layerInputs[[2]],layerOutputDelta[[2]]]};
WeightDec[Composite[{net1_,net2_}],grad_]:=Composite[{WeightDec[net1,grad[[1]]],WeightDec[net2,grad[[2]]]}];


(*UNTESTED*)
SyntaxInformation[DeCompose]={"ArgumentsPattern"->{_}};
ForwardPropogateLayer[inputs_,DeCompose[size_]]:=
(* We want to take only the final sublayer in previous subnetwork to feed forward *)
   MapThread[Join,{z1=inputs[[1,-1]],z2=inputs[[2,-1]]}];
BackPropogateLayer[DeCompose[size_],postLayerDeltaA_,inputs_,outputs_,previousLayerNet_]:={
   BackPropogateLayers[t1=previousLayerNet[[1,1]],t2=inputs[[1]],t3=postLayerDeltaA[[All,1;;size]]],
   BackPropogateLayers[t4=previousLayerNet[[1,2]],t5=inputs[[2]],t6=postLayerDeltaA[[All,size+1;;-1]]]};
GradLayer[DeCompose[size_],layerInputs_,layerOutputDelta_]:={};
WeightDec[DeCompose[size_],grad_]:=DeCompose[size];
